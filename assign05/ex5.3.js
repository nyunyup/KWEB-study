const express = require('express');
const runQuery = require('./database')

const app = express();
const port = 3000;


app.get('/fare', async (req, res, next) => {
    try {
        const { uid } = req.query;
        const qry = 'SELECT users.name, Sum(Round(types.fare_rate / 100 * trains.distance / 10, -2)) AS cost' +
                    'FROM tickets' +
                    'INNER JOIN users ON tickets.user=users.id and users.id=?' +
                    'INNER JOIN trains ON tickets.train=trains.id' +
                    'INNER JOIN types ON trains.type=types.id';
        const { name, cost } = (await runQuery(qry, [uid]))[0];
        return res.send('Cost of ${name} is ${cost} KRW.');
    }

    catch (err) {
        console.error(err);
        return res.sendStatus(500);
    }
});

app.get('/train/status', async (req, res, next) => {
    try {
        const { tid } = req.query;
        const qry = 'SELECT Count(*) AS occupied, max_seats AS maximum FROM tickets' +
            'INNER JOIN trains ON trains.id = tickets.train AND trains.id = ?' +
            'INNER JOIN types ON trains.type = types.id';
        const { occupied, maximum } = (await runQuery(sql, [tid]))[0];
        const av_seat = occupied < maximum;
        return res.send(`Train ${tid} is ${av_seat ? 'not ' : ''}sold out`);
    }

    catch (err) {
        console.error(err);
        return res.sendStatus(500);
    }
});

app.listen(port, () => console.log(`Server listening on port ${port}!`));